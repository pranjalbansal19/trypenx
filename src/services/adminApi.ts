...
// Scopes API
export async function getScopesByCustomerId(customerId: string): Promise<Scope[]> {
	await delay();
	return scopesStore.filter((s) => s.customerId === customerId);
}

export async function createScope(data: Omit<Scope, 'id' | 'createdAt'>): Promise<Scope> {
	await delay();
	const scope: Scope = {
		...data,
		id: generateId(),
...
export async function updateScope(id: string, data: Partial<Scope>): Promise<Scope> {
	await delay();
...
export async function getTestConfigByCustomerId(customerId: string): Promise<TestConfiguration | null> {
	await delay();
...
export async function createTestConfig(
	...
	testConfigsStore = testConfigsStore.filter((tc) => tc.customerId !== data.customerId);
...
// Test Runs API
export async function getTestRunsByCustomerId(customerId: string): Promise<TestRun[]> {
	await delay();
	return testRunsStore.filter((tr) => tr.customerId === customerId).sort((a, b) => 
...
export async function getAllTestRuns(): Promise<TestRun[]> {
	await delay();
	return [...testRunsStore].sort((a, b) => 
		new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
	);
}

export async function createTestRun(data: Omit<TestRun, 'id' | 'createdAt'>): Promise<TestRun> {
	await delay();
...
// Reports API
export async function getReportsByCustomerId(customerId: string): Promise<Report[]> {
	await delay();
	return reportsStore.filter((r) => r.customerId === customerId).sort((a, b) => 
		new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
	);
}
...
export async function getCustomerRunInfo(customerId: string): Promise<{
	...
	const completedRuns = runs.filter((r) => r.status === 'Completed').sort((a, b) => 
		new Date(b.endTime || b.createdAt).getTime() - new Date(a.endTime || a.createdAt).getTime()
	);
	const scheduledRuns = runs.filter((r) => r.status === 'Scheduled').sort((a, b) => 
